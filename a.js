...
"use strict";
{
			...
			gl.shaderSource(
				...
				`
					...
					void main() {
						// 3D Rotations
						float tx, ty, tz;
						tx = uRotation.z * aPosition.x - uRotation.w * (aPosition.z - 100.0);
						tz = uRotation.w * aPosition.x + uRotation.z * (aPosition.z + 0.0);
	...
	const load = img => {
		...
		const push = (x, y) => {
			const p = (y * width + x) * 4;
			const r = bitmap[p + 0] / brightness;
			const g = bitmap[p + 1] / brightness;
			const b = bitmap[p + 2] / brightness;
			colors.push(r);
			colors.push(g);
			colors.push(b);
		...
		for (let x = 0; x < width; x += step) {
			if (d > 0) {
				for (let y = 0; y < height; y += step) push(x, y);
			} else {
				for (let y = height; y > 0; y -= step) push(x, y);
			}
			d = -d;
		}
		// load buffers to gpu
